...
export class fDocument {
    ...
    async get() {
        if (!this.deleted) {
            const docRef = doc(db, this.template["___name"], this.id);
    ...
    async delete() {
        await deleteDoc(doc(db, this.template["___name"],  this.id));
        this.deleted = true;
    }
    ...
    async update() {
        ...
        if (!(diff && Object.keys(diff).length === 0 && Object.getPrototypeOf(diff) === Object.prototype)) {
            ...
            try {
                await updateDoc(doc(db, this.template["___name"], this.id), updatedDoc);
            } catch (error) {
                console.log(error)
            }
        }
